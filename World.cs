...
//        @synthesize dropBoxVariables;
        ...
        public World () //CrashLandingAppDelegate mainThing)
		{
			...
			inFirstLoad = true;
			string language = "en";//(NSLocale.PreferredLanguages()).ObjectAtIndex(0);
            if (language == ("de")) {
     ...
     //       #if SIMULATOR_PERFORMANCE_TEST
       //         this.TestInit();
        ...
        public Texture2D_Ross LoadTextureP1(bool stretchable, string textureName)
        {
			return this.LoadTextureP1P2(stretchable,textureName,Texture2D_RossPixelFormat.t_Automatic);
...
/*            if (inFirstLoad) Globals.g_main.RenderLoadingBar();
        ...
        public Texture2D_Ross LoadTextureP1P2(bool stretchable, string textureName, Texture2D_RossPixelFormat forceType)
        {
            if (inFirstLoad) Globals.g_main.RenderLoadingBar();
           ...
           // if (!iPhone4) {
//				useName = "/done/" + textureName;////.Copy();
            //}

//textureName.StringByDeletingPathExtension();

			
//			return new Texture2D_Ross(stretchable, false, -1, useName, forceType);
			...
			return new Texture2D_Ross(stretchable, textureName, withTransparency);
        }
        ...
        string GetStringChinese (StringId inStringId)
		{
            ...
            case StringId.kString_EA :
                return "购买或/以及使用这款程序，表示您同意最终用户许可条款";
        ...
        string GetStringJapanese(StringId inStringId)
        {
            ...
            case StringId.kString_EA :
                return
"このアプリケーションの購入もしくは使用でエンドユーザ使用許諾契約書及びEAのプライバシーポリシー及び利用規約に同意した物とみなします。"
                ;
        ...
        public void SetupFontSubTextures(List<int> inArray)
        {
            ...
            letterX -= widthExclamation;
            this.AddRowOfSubtexturesP1P2P3P4P5(inArray, 1, (int)kWidth, (int)kHeight, (int)letterX, 0);
            this.AddRowOfSubtexturesP1P2P3P4P5(inArray, 5, (int)kWidth, (int)kHeight + 4, 0, 4 + ((int)kHeight * 2));
        }
        ...
        public void SetupAtlasFromPListP1P2P3(bool doubleSize, List<int> gridArray, string inName, bool fromDropbox)
        {
		   // UnityEngine.Debug.Log (string.Format("Setting Up Atlas from PList {0}",inName));
            ...
            if (Globals.useRetina) {
                ...
                useName = String.Format("%@@2x~iphone.plist", temp);
                iPhone4 = true;
            }
...
//            NSDictionary frames;
			...
			plistDict = (Dictionary<string, object>)Plist.readPlist("Assets/Resources/done/" + inName);
...
//            if (!g_myViewController.IsLoadingAtlas((int) AtlasType.kAtlas_RainbowParticles)) {
						atlas [(int)AtlasType.kAtlas_RainbowParticles] = new ZAtlas ();
						...
						atlas [(int)AtlasType.kAtlas_FontColours] = new ZAtlas ();
		...
		/*				#if DEBUG_DRAW_RACING_LINE
                ...
                atlas[(int) AtlasType.kAtlas_Debug] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_Huh].initAtlasWithString(this, false, 0, 0, gridArray, "AtlasDebug.png");
						...
						if (Globals.useRetina) {
								atlas [(int)AtlasType.kAtlas_FontNumbers] = new ZAtlas (); 
            ...
            if (Globals.useRetina) 
			{
				atlas[(int) AtlasType.kAtlas_321] = new ZAtlas(); 
            ...
            this.SetupAtlasFromPListP1P2(false, gridArray, "HudAtlas.plist");
            atlas[(int) AtlasType.kAtlas_Hud] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_Hud].initAtlasWithString(this, true, 0, 0, gridArray, "HudAtlas.png", (int) Texture2D_RossPixelFormat.t_RGBA8888);
        }
        ...
        public void LoadRaceLoseAtlas()
        {
            ...
            if (atlas[(int) AtlasType.kAtlas_RaceLose] == null) {
                ...
                this.SetupAtlasFromPListP1P2(false, gridArray, "RaceLoseAtlas.plist");
                atlas[(int) AtlasType.kAtlas_RaceLose] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_RaceLose].initAtlasWithString(this, true, 0, 0, gridArray, "RaceLoseAtlas.png", (int) Texture2D_RossPixelFormat.t_RGBA8888);
            }
        ...
        public void LoadRaceWinAtlas()
        {
            ...
            if (atlas[(int) AtlasType.kAtlas_ShowLevelAndTip] == null) {
                this.ReleaseAtlasIfLoaded(AtlasType.kAtlas_RaceLose);
                this.SetupAtlasFromPListP1P2(false, gridArray, "ShowLevelAndTipAtlas.plist");
                atlas[(int) AtlasType.kAtlas_ShowLevelAndTip] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_ShowLevelAndTip].initAtlasWithString(this, true, 0, 0, gridArray, "ShowLevelAndTipAtlas.png", (int) Texture2D_RossPixelFormat.
                  t_RGBA8888);
            }

            if (atlas[(int) AtlasType.kAtlas_AppleSign] == null) {
                this.SetupAtlasFromPListP1P2(false, gridArray, "AppleSignAtlas.plist");
                atlas[(int) AtlasType.kAtlas_AppleSign] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_AppleSign].initAtlasWithString(this, true, 0, 0, gridArray, "AppleSignAtlas.png", (int) Texture2D_RossPixelFormat.t_RGBA8888);
            }
        ...
        public void LoadPlayersAtlas()
        {
            ...
            List<int> gridArray = new List<int>();// = new List<int>(0);
            this.SetupAtlasFromPListP1P2(false, gridArray, "PiggyAnims.plist");
            atlas[(int) AtlasType.kAtlas_PiggyAnims] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_PiggyAnims].initAtlasWithString(this, true, 0, 0, gridArray, "PiggyAnims.png", (int) Texture2D_RossPixelFormat.t_RGBA8888);
        }

        public void RemovePlayersAtlas()
        {
            Globals.Assert(atlas[(int) AtlasType.kAtlas_PiggyAnims] != null);
            //atlas[(int) AtlasType.kAtlas_PiggyAnims];
        ...
        public void ReleaseFrontEndAtlases()
        {
           ...
           // atlas[(int) AtlasType.kAtlas_FrontendAndShowlevel];
        ...
        {
            if (atlas[(int) AtlasType.kAtlas_ShowLevelAndTip] != null) {
              //  atlas[(int) AtlasType.kAtlas_ShowLevelAndTip];
                atlas[(int) AtlasType.kAtlas_ShowLevelAndTip] = null;
            }

            if (atlas[(int) AtlasType.kAtlas_AppleSign] != null) {
               // atlas[(int) AtlasType.kAtlas_AppleSign];
        ...
        public void LoadFeelGoodAtlas()
        {
            if (atlas[(int) AtlasType.kAtlas_FeelGood] == null) {
                List<int> gridArray = new List<int>();// = new List<int>(0);
                this.SetupAtlasFromPListP1P2(false, gridArray, "FeelGoodAtlas.plist");
                atlas[(int) AtlasType.kAtlas_FeelGood] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_FeelGood].initAtlasWithString(this, true, 0, 0, gridArray, "FeelGoodAtlas.png", (int) Texture2D_RossPixelFormat.t_RGBA8888);
            }
        ...
        public void LoadAnotherPiggyAtlases()
        {
            if (atlas[(int) AtlasType.kAtlas_AnotherPiggy] == null) {
                List<int> gridArray = new List<int>();// = new List<int>(0);
                this.SetupAtlasFromPListP1P2(false, gridArray, "AnotherPiggy.plist");
                atlas[(int) AtlasType.kAtlas_AnotherPiggy] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_AnotherPiggy].initAtlasWithString(this, true, 0, 0, gridArray, "AnotherPiggy.png", (int) Texture2D_RossPixelFormat.t_RGBA8888)
                  ;
            }
        ...
        public void LoadFrontEndAtlases()
        {
            List<int> gridArray = new List<int>();// = new List<int>(0);
            Globals.Assert(atlas[(int) AtlasType.kAtlas_FrontendAndShowlevel] == null);
            this.SetupAtlasFromPListP1P2(true, gridArray, "ButtonsAtlas.plist");
            atlas[(int) AtlasType.kAtlas_FrontendAndShowlevel] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_FrontendAndShowlevel].initAtlasWithString(this, true, 0, 0, gridArray, "ButtonsAtlas.png", (int) Texture2D_RossPixelFormat.
              t_RGBA8888);
        }

        public void ReleaseAtlasForLevelBuilder_Ross()
        {
          //  atlas[(int) AtlasType.kAtlas_CommonLevelBuilder_Ross];
        ...
        {
            int tileSize = 64;
            if (Globals.useRetina) tileSize = 128;

            Globals.Assert(atlas[(int) AtlasType.kAtlas_CommonLevelBuilder_Ross] == null);
            atlas[(int) AtlasType.kAtlas_CommonLevelBuilder_Ross] = new ZAtlas(); 
			atlas[(int) AtlasType.kAtlas_CommonLevelBuilder_Ross].initAtlasWithString(this, false, tileSize, tileSize, null, "AtlasCommonLevelBuilder_Ross.png", (int)
        ...
        {
            List<int> gridArray = new List<int>();// = new List<int>(0);
            if (Globals.useRetina) gridArray.Add((int)(64));
            ...
            atlas[(int) AtlasType.kAtlas_ParticlesScene] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_ParticlesScene].initAtlasWithString(this, true, 16, 16, gridArray, "AtlasParticles.png");

            int tileSize = 64;
            if (Globals.useRetina) tileSize = 128;

                Globals.Assert(atlas[(int) AtlasType.kAtlas_GrassTiles] == null);
                atlas[(int) AtlasType.kAtlas_GrassTiles] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_GrassTiles].initAtlasWithString(this, true, tileSize, tileSize, null, "AtlasGrass.png", (int) Texture2D_RossPixelFormat.
                  t_RGB565);
            ...
            atlas[(int) AtlasType.kAtlas_GrassSpriteTiles] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_GrassSpriteTiles].initAtlasWithString(this, true, tileSize, tileSize, null, "AtlasGrass_SpriteTiles.png", (int)
              Texture2D_RossPixelFormat.t_RGBA8888);
                Globals.Assert(atlas[(int) AtlasType.kAtlas_GameThingsGrass] == null);
                this.SetupAtlasFromPListP1P2(false, gridArray, "GameThingsGrass.plist");
                atlas[(int) AtlasType.kAtlas_GameThingsGrass] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_GameThingsGrass].initAtlasWithString(this, true, 0, 0, gridArray, "GameThingsGrass.png", (int) Texture2D_RossPixelFormat.
                  t_RGBA8888);
...
//                atlas[releaseList[i]];
        ...
        public void LoadMudAtlases()
        {
            List<int> gridArray = new List<int>();// = new List<int>(0);
            gridArray.Clear();//RemoveAllObjects();
            if (Globals.useRetina) gridArray.Add((int)(2));
            ...
            this.AddSubtextureToAtlasP1P2P3P4(gridArray, 128, 240, 144, 256);
                Globals.Assert(atlas[(int) AtlasType.kAtlas_ParticlesScene] == null);
                atlas[(int) AtlasType.kAtlas_ParticlesScene] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_ParticlesScene].initAtlasWithString(this, true, 32, 32, gridArray, "AtlasParticlesFarm.png");

            int tileSize = 64;
            if (Globals.useRetina) tileSize = 128;

                Globals.Assert(atlas[(int) AtlasType.kAtlas_MudTiles] == null);
                atlas[(int) AtlasType.kAtlas_MudTiles] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_MudTiles].initAtlasWithString(this, true, tileSize, tileSize, null, "AtlasMud.png", (int) Texture2D_RossPixelFormat.t_RGB565);

            Globals.Assert(atlas[(int) AtlasType.kAtlas_MudSpriteTiles] == null);
            atlas[(int) AtlasType.kAtlas_MudSpriteTiles] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_MudSpriteTiles].initAtlasWithString(this, true, tileSize, tileSize, null, "AtlasMud_SpriteTiles.png", (int)
              Texture2D_RossPixelFormat.t_RGBA8888);
            gridArray.Clear();//RemoveAllObjects();
            gridArray.Add((int)(1));
			...
			Globals.Assert(atlas[(int) AtlasType.kAtlas_GameThingsMud] == null);
                this.SetupAtlasFromPListP1P2(false, gridArray, "GameThingsMud.plist");
                atlas[(int) AtlasType.kAtlas_GameThingsMud] = new ZAtlas(); atlas[(int) AtlasType.kAtlas_GameThingsMud].initAtlasWithString(this, true, 0, 0, gridArray, "GameThingsMud.png");
            ...
            if (true) {
                    ...
                    this.LoadShowLevelAtlases();
                        ...
                        if (Constants.STARTING_LEVEL < 8) {
                            ...
                            (game.lBuilder != null).SetCurrentScene((int) SceneType.kSceneGrass);
                        }
                        else {
                            frontEnd.SetSelectedStage((int) SceneType.kSceneMud);
                            (game.lBuilder != null).SetCurrentScene((int) SceneType.kSceneMud);
                        }
  ...
  //          value = (UIDevice.CurrentDevice()).name;
    //        value = (UIDevice.CurrentDevice()).SystemName();
      //      value = (UIDevice.CurrentDevice()).SystemVersion();
            ...
            if (Globals.useRetina) artLevel = 2;
            ...
            (Ztransition.GetTransition()).SetPBackTexture(game.GetTexture((TextureType) TextureType.kTexture_Whiteout));
            (Ztransition.GetTransition()).SetPTexture(game.GetTexture((TextureType) TextureType.kTexture_HudLoading));
...
//            this.InitReachability();
        ...
        public void SetupPullTab()
        {
            ...
            info.texture = this.LoadTextureP1(false, "leaderBoardRank.png");
